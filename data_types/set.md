...
Запись:
`x ∈ X` ( `x ∉ X` ) означает, что `x` **является** (**не является**) элементом множества `X`.
...
<img src="../img/data_types/set5.jpg" width="400px">
...
Комплексным числом в математике называется любое число, представимое в виде a+bi, где a и
b – вещественные числа, а `i = корень из −1` – мнимая единица. Множество комплексных чисел обозначается буквой C.
