...
## Шаг 7: Удаление последнего элемента
- Реализуйте метод `remove_last(self)`.
- Внутри метода:
    - Если `self.head` равен `None`, это означает, что `список пуст`.
    - Завершите выполнение метода, так как удаление невозможно.
    - Проверьте, есть ли только один узел в списке, проверив, равен ли `self.head.next` `None`.
    - Установите `self.head` в `None`, чтобы удалить единственный элемент из списка.
    - Завершите выполнение метода, так как удаление завершено.
    - Создайте переменную `current` и установите её на голову списка.
    - Запустите цикл `while`, который будет продолжаться, пока `current.next.next` не равен `None`.
    - Внутри цикла переместите `current` на следующий узел, чтобы продолжить поиск.
    - Установите `current.next` в `None`, чтобы удалить ссылку на последний узел, тем самым удаляя его из списка.
- **Схема работы**:
    ```
    Изначальный список:
        head -> [ data: 30 | next: -> [ data: 20 | next: -> [ data: 10 | next: None ] ] ]
    ...
    После удаления 10:
        head -> [ data: 30 | next: -> [ data: 20 | next: None ] ]

    После удаления 20:
        head -> [ data: 30 | next: None ]
...
def remove_last(self):
    ...
    """
    if not self.head:  # Проверяем, пуст ли список
        return  # Завершаем выполнение метода, так как нечего удалять
    if not self.head.next:  # Проверяем, является ли единственный узел головой
        self.head = None  # Удаляем единственный элемент, устанавливая голову в None
        return  # Завершаем выполнение метода

    # Проходим до предпоследнего узла
    current = self.head  # Начинаем с головы списка
    while current.next.next:  # Пока есть следующий узел и следующий за ним узел
        current = current.next  # Переходим к следующему узлу
    current.next = None  # Удаляем ссылку на последний узел, тем самым удаляя его из списка
```